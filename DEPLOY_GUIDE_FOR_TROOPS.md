...
- 點擊 **「⬇️ 下載 Code.gs (最新唯一版本)」** → 得 `Code.gs` (約27KB, 含細緻權限+私隱開關)
...
2. 去 **📋審批申請** (已合併入 **✅審批中心** → 用戶審批) 批准第一個管理員申請（或直接在用戶管理新增）

### 建立管理員帳號

1. 回首頁選旅團 → 點「還沒有帳號？申請加入」→ 填 YMIS 10位、姓名、申請角色「管理員」→ 提交
2. 用管理員帳號登入 → 審批中心 → 用戶審批 → 批准
3. 登出，用新管理員帳號登入 (預設密碼 `changeme`，立即更改)
...
**Q: 超管帳號會被其他人看到嗎？**
A: 不會。只有超管登入才看到測試工具及超管角色說明，普通用戶在用戶管理看不到超管那行，已過濾。
