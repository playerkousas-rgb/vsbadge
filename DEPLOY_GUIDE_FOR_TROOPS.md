...
- 超管：登入帳號 `sheep`（用戶管理欄）／電郵 `sheep@vsbadge.local` / 密碼 `0728` / 角色 super_admin（可直接登入，不會因重新初始化而消失）
...
>
> 🔢 密碼原則（v8.3）：密碼最短 **4 位**；批量開戶／審批初始密碼預設為 **1234**（首次登入仍會要求更改）。若你希望恢復曾消失的超管 sheep，只需覆蓋 `Code.gs` 並重新部署，再執行一次 `initializeSheets()`，系統會自動補回該帳號。
