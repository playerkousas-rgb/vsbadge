...
> v4.0.0 對齊 ROVERBADGE v4.0.0 發佈編號；Apps Script 的 v8.x 是後端功能版本，兩者用途不同。
>
> 本版新增免登入的「回報問題／意見」表單。Proxy 會把問題回報或意見回饋轉成 Scout Admin 統一回報格式 v1（`sourceApp: vsbadge`、問題 `type: issue` 或意見 `type: feedback`，以及相應內容欄位），送到固定中央收件匣 `SCOUT_ADMIN_API`；只有收件匣回覆已接收，前端才顯示成功。
>
> 登入或使用遇到問題時，可透過「回報問題／意見」留下聯絡方式；送出後會顯示「已傳送給開發者，請等待通知」。
>
> Apps Script v4.0.0 升級：覆蓋 `apps-script/Code.gs` 後，在每個既有旅團試算表**執行一次 `initializeSheets()`**，再重新部署為新版本。這次初始化只清除／匿名化舊版保留帳戶殘留，不刪除一般用戶、進度、審批或履歷資料。直接入口狀態未設定時維持開啟，閂口控制只在 Apps Script／上游，不在下游介面顯示。
