...
8. **不要執行 `initializeSheets()`**、不要改工作表欄位、不要清空 Tokens／Users。這次更新沒有 schema migration，也不會在升級時刪除既有列。
9. 確認成員／領袖既有帳號可正常登入，進度、審批及管理功能正常。
...
不改工作表名稱、欄位、既有進度／用戶／履歷資料，不重跑初始化。正常登入仍會像原版一樣在既有 Tokens 工作表新增登入紀錄；正常業務操作的資料寫入維持不變。
