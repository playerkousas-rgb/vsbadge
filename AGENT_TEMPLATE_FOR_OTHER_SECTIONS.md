...
### 深資童軍 & 樂行童軍（有自治架構）
- 深資：執行委員會 (Executive Committee) - 自務自治
- 樂行：團管理委員會 (Management Committee) - 名稱不同，功能類似
- 需要細緻權限：成員→執委，由團長設定可考核什麼（會員章/活動段章細項不用/其他逐個勾）
- 領袖默認全部(*)，團長/管理員可改低層級
- 私隱開關：allow_member_view_others，成員預設只看自己

### 童軍 & 幼童軍（全部由領袖負責）
- 沒有執行委員會/管理委員會，全部由領袖負責
- **不需要做這種權限設定**，簡化為：
  - member：只能看自己，只能申請
  - leader (branch_leader/group_leader/admin)：可勾選全部，可批量，可審批
- 用戶管理只需角色切換，不需⚙️權限細分Modal
- 可刪除 `allowed_badges` 相關邏輯，或保留但默認簡化
- 私隱開關可保留，但預設關閉更合理（幼童軍家長更重私隱）

Agent 在做童軍/幼童軍時，請將 `loadUsers()` 中的權限按鈕隱藏，`setupTabsByRole()` 簡化，`canCurrentUserTickItem()` 直接按 role 判斷即可。
